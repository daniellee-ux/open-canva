...
- **Emit logical groupings.** Wrap each self-contained unit — a card (its bg + heading + body + icon), a stat (number + label), a header cluster (logo + wordmark), a row of bullets — in a `<Group label="…">`. Give the group a stable `label` (and `id` if you reference it). This makes the layers panel readable, lets the user select/move the whole unit at once, and keeps the layout robust. Prefer a handful of meaningful groups over a flat list of 30 absolutely-positioned objects.
