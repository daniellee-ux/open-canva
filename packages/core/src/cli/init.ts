...
export async function init(targetArg?: string): Promise<void> {
  ...
  if (existsSync(gi)) {
    if (existsSync(dotGi)) {
      ...
      if (add.length) {
        const header = '# Added by opencanva init (.claude/ holds Claude-specific skills, regenerated by `npm run sync`)';
        writeFileSync(dotGi, `${existing.replace(/\n*$/, '')}\n\n${header}\n${add.join('\n')}\n`);
      }
      rmSync(gi);
    } else {
      renameSync(gi, dotGi);
  ...
  // 0.0.0 is the unpublished placeholder version: npm install can't resolve the
  // pinned dep from the registry, so surface the link step inline before it fails.
  // (Exact 0.0.0 only — a real 0.0.x publish IS installable.)
  if (corePkg.version === '0.0.0') {
