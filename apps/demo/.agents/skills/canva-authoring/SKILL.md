...
- **Factor reusable subcomponents in code.** When the same shape repeats (a stat tile, a list row, a chip), write a small local component whose children sit at coordinates **relative to (0,0)** and that takes **no** `x`/`y` of its own — e.g. `function StatTile({ value, label }) { return (<><Text x={0} y={0}>{value}</Text><Text x={0} y={48}>{label}</Text></>) }`. Then place each instance inside its **own** positioned group with **literal** numbers: `<Group x={120} y={80}><StatTile … /></Group>`. Do **not** thread position through a prop (`<Group x={x} y={y}>`) — every instance shares that one source line, so when the user drags one the inspector rewrites the literal there and collapses them all onto the same spot. A shared component in source IS the reusable component; there is no symbol/instance system. (The shipped `start-here` design's `StepBody` is exactly this pattern.)
